...
const STORE_META = {
  ...
  serper:  { label: "Google Shopping", emoji: "🌐" },
  bestbuy: { label: "Best Buy",      emoji: "💙" },
  walmart: { label: "Walmart",       emoji: "🔵" },
};

function CountryStoreNotice({ countryCode }) {
  const available = storesForCountry(countryCode);
  const storeChips = available.map(key => STORE_META[key]).filter(Boolean);

  return (
    <div style={{
      background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: 8,
      padding: "8px 14px", fontSize: 12, color: "#0369a1", marginBottom: 10,
      display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap"
    }}>
      <span style={{ fontWeight: 600 }}>🛒 Searching:</span>
      {storeChips.map(s => (
        <span key={s.label} style={{
          background: "#fff", border: "1px solid #bae6fd", borderRadius: 20,
          padding: "2px 10px", fontWeight: 600, color: "#0369a1", fontSize: 12
        }}>
          {s.emoji} {s.label}
        </span>
      ))}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f9fafb", fontFamily: "sans-serif" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "16px" }}>
        ...
        <CountryStoreNotice countryCode={selectedCountry} />
