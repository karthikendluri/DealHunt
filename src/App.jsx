...
// ─── AMAZON (via Serper shopping with amazon source filter) ─
async function amazonSearch(query, withDiscount) {
  const q = withDiscount ? `${query} deal discount` : query;
  const cacheKey = `amazon__${q}__${withDiscount}`;
  const cached = getCached(cacheKey);
  if (cached) return cached;
  ...
  try {
    const res = await fetch("https://google.serper.dev/shopping", {
      method: "POST",
      headers: { "X-API-KEY": SERPER_KEY, "Content-Type": "application/json" },
      body: JSON.stringify({ q: `${q} site:amazon.com`, gl: "us", num: 20 }),
    });
    const data = await res.json();
    if (!data.shopping) return [];
    ...
    const results = data.shopping
      ...
      .map(item => {
        const price = parseFloat(item.price?.replace(/[^\d.]/g, "")) || 0;
        // Try to extract original/list price from title or delivery string
        // Amazon often shows crossed-out prices in the snippet
        const originalRaw = item.delivery
          ? parseFloat(item.delivery.replace(/[^\d.]/g, "")) || 0
          : 0;
        const original = originalRaw > price ? originalRaw : (withDiscount ? price * 1.15 : price);
        ...
        return {
          ...
          discounted_price: price,
          discount_percent: discount,
          url: item.link || item.product_link || "https://amazon.com",
...
export default function App() {
  ...
  const search = async () => {
    ...
    setLoading(true);
    // Now 7 API calls: added 2 Amazon searches (with/without discount)
    setApiCallCount(c => c + 7);
    try {
      const [general, bestbuy, walmart, ebay, generalNd, amazonDiscount, amazonFull] = await Promise.all([
        serperSearch(query, location, true),
        bestBuySearch(query),
        walmartSearch(query),
        ebaySearch(query),
        serperSearch(query, location, false),
        amazonSearch(query, true),
        amazonSearch(query, false),
      ]);
      setDeals([...amazonDiscount, ...bestbuy, ...walmart, ...ebay, ...general]);
      setNoDiscountDeals([...amazonFull, ...bestbuy, ...walmart, ...ebay, ...generalNd]);
  ...
  // Build store dropdown options with live counts — now includes Amazon
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "16px" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 16, alignItems: "center" }}>
          <input
            ...
            placeholder="Search Amazon, Best Buy, Walmart, eBay & more..."
          ...
          <>
            ...
            {loading && (
              <div style={{ textAlign: "center", marginTop: 40 }}>
                <div style={{ fontSize: 16, marginBottom: 8 }}>🔍 Searching Amazon, Best Buy, Walmart, eBay...</div>
            ...
            {!loading && !hasSearched && (
              <div style={{ textAlign: "center", marginTop: 80, color: "#9ca3af" }}>
                ...
                <div style={{ fontSize: 16 }}>Search deals across Amazon, Best Buy, Walmart, eBay & more</div>
                <div style={{ fontSize: 13, marginTop: 8, color: "#d1d5db" }}>🟠 Amazon &nbsp;•&nbsp; 💙 Best Buy &nbsp;•&nbsp; 🔵 Walmart &nbsp;•&nbsp; 🔴 eBay</div>
              </div>
            )}
          </>
        )}
            ...
            {hasSearched && (
              <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: "12px 16px", marginBottom: 12, display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span style={{ fontSize: 13, color: "#475569" }}>
                  <strong>Full-price listings</strong> from Amazon, Best Buy, Walmart, eBay & more.
                </span>
              </div>
            )}
